...
When the user asks you to open an app, click something, control Spotify, or interact with anything on the computer: \
first try list_windows to see what is open, then focus_window to bring it forward, then find_and_click to click buttons by name — \
this works without any screenshot for most standard Windows apps. \
Only fall back to take_screenshot + mouse_click with coordinates for apps that do not expose accessibility info (games, some web content). \
Use keyboard_type to type text after clicking an input field, and key_press for shortcuts like Enter or Escape.
